      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        ...
        {!isLinked ? (
          <div className="text-center">
            ...
            <a href={`${API}/auth/lwa`} className="inline-block bg-amber-400 hover:bg-amber-500 text-slate-900 font-bold px-8 py-3 rounded-xl transition-colors w-full text-center">
              Connect Amazon Account
            </a>
            <p className="text-xs text-slate-400 mt-4">We only request your profile and device access. No passwords stored.</p>
          </div>
        ) : (
          <div className="space-y-5">
            <div className="flex items-center gap-2 text-green-600 text-sm bg-green-50 rounded-lg p-3">
              <span>✅</span><span>Amazon account linked successfully!</span>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Select Your City</label>
              <select value={cityId} onChange={e => setCityId(e.target.value)} className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm">
            ...
            <div>
              ...
              <select value={method} onChange={e => setMethod(e.target.value)} className="w-full border border-slate-300 rounded-lg px-3 py-2.5 text-sm">
                {METHODS.map(m => (<option key={m.value} value={m.value}>{m.label}</option>))}
              </select>
            </div>
            {error && <p className="text-red-500 text-sm bg-red-50 rounded-lg p-3">{error}</p>}
            <button onClick={handleSave} disabled={!cityId || saving} className="w-full bg-slate-800 hover:bg-slate-700 disabled:opacity-40 text-white font-semibold py-3 rounded-xl transition-colors">
...
export default function ConnectPage() {
  return (
    <Suspense fallback={<main className="min-h-screen bg-slate-900 flex items-center justify-center"><div className="text-white">Loading...</div></main>}>
      <ConnectForm />
    </Suspense>
  )
}
